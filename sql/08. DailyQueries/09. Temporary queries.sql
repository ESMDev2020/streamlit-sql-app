...
--SELECT * FROM [mrs].[01_mysql_MSSQL_Equivalents]
SELECT * FROM [mrs].[mysql_____ap]


SELECT * FROM [mrs].[mysql_____ap]
WHERE [PO_______PO__] = '01968207'


SELECT COUNT(*) FROM [mrs].[mysql_____ap]
WHERE [PO_______PO__] <> ''
group by DATE_____DATE  
...
SELECT 
	YEAR(DATE_____DATE) AS Year,
	COUNT(*) AS Count
FROM [mrs].[mysql_____ap]
WHERE [PO_______PO__] <> ''
GROUP BY YEAR(DATE_____DATE)
ORDER BY Year

/***********************************
SHIPMENT DOLLARS PER YEAR
************************************/

SELECT 
	YEAR(DATE_____DATE) AS Year,
	FORMAT(SUM(CONVERT(FLOAT, AMOUNT_____AMOUNT))/100, 'N0') AS TotalAmount
FROM [mrs].[mysql_____ap]
WHERE [PO_______PO__] <> ''
GROUP BY YEAR(DATE_____DATE)
ORDER BY Year;
...
FROM [mrs].[mysql_____ap]
WHERE [PO_______PO__] <> ''
AND		YEAR(DATE_____DATE) = 2025
--ORDER BY Year

VNAME, BDITEM, RevisedPromiseDate, --BDOQO, Current_inv._quantity_ordered, original_ordered_quantity, Total_inventory_quantity_received_,purchaseOrderDate, ReceivedDate

CM.[Customer_Alpha_Name_____CALPHA]


--We link one sales order, or purchase order, or service purchase order to this purchaseorder
SELECT dw.[PO_______PO__] AS DocuwarePO FROM [mrs].[mysql_____ap] dw WHERE [PO_______PO__] = '01968207'
SELECT dw.[PO_______PO__] AS DocuwarePO, * FROM [mrs].[mysql_____ap] dw WHERE [PO_______PO__] = '01968207'


[OEDETAIL],[OEOPNORD], [PODETAIL],[SPHEADER]
[z_Order_Detail_File_____OEDETAIL],
[z_Open_Order_File_____OEOPNORD], 
[z_Purchase_Order_Detail_File_____PODETAIL],
[z_Service_Purchase_Order_Header_File_____SPHEADER]



SELECT TOP 10 * FROM	[mrs].[z_Order_Detail_File_____OEDETAIL] WHERE [Transaction_#_____ODORDR] like '%01968207%'			--NO
SELECT TOP 10 * FROM	[mrs].[z_Order_Detail_File_____OEDETAIL] WHERE [INBOUND_ORDER_NUMBER_____ODIBOR] like '%01968207%'	--NO
SELECT TOP 10 * FROM	[mrs].[z_Open_Order_File_____OEOPNORD] WHERE [ORDER_NUMBER_____OOORDR] Like '%01968207%'			--NO
SELECT TOP 10 * FROM	[mrs].[z_Purchase_Order_Detail_File_____PODETAIL] WHERE [Transaction_#_____BDPONO] Like '%01968207%'
SELECT TOP 10 * FROM	[mrs].[z_Service_Purchase_Order_Header_File_____SPHEADER] WHERE [Transaction_#_____BSORDR] Like '%01968207%'
