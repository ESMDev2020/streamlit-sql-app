...
VERSION
	9 (Corrected SortOrder/Flag assignment for Date Suffix vs Flag types)
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            BEGIN TRY
                ...
                -- Determine sort order (Revised v6.13 - Corrected Flag/Date Suffix order)
                SET @SortOrder = CASE
                    ...
                    -- Priority 2: Data Content Characteristics (Single Value, Flag(%RECD), Flag(2), Whitespace)
                    WHEN @DistinctCount = 1 THEN 7 -- Single Value
                    WHEN @CurrentColumn LIKE '%RECD' THEN 8 -- Flag (%RECD)
                    WHEN @DistinctCount = 2 THEN 10 -- Flag (2 Values) <<< NEW ORDER
                    WHEN @DistinctCount = 0 AND @WhitespaceCount > 0 THEN 12 -- All Whitespace

                    -- Priority 3: Name Patterns (FK/Code, Date Suffix - if not caught above)
                    WHEN (@CurrentColumn LIKE '%id' OR @CurrentColumn LIKE '%code%' OR @CurrentColumn LIKE '%key%') THEN 2 -- FK/Code (Potential)
                    WHEN (@CurrentColumn LIKE '%CC' OR @CurrentColumn LIKE '%DD' OR @CurrentColumn LIKE '%MM' OR @CurrentColumn LIKE '%YY') THEN 11 -- Date Suffix
                    ...
                    WHEN @CurrentDataType IN ('float', 'real') THEN 9 -- Float <<< Renumbered
                ...
                -- Override IsUniqueFlag if SortOrder indicates a Flag type (Corrected to 8 and 10)
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            CASE
                ...
                WHEN SortOrder = 8 THEN 'Flag (%RECD)' -- Corrected Order
                WHEN SortOrder = 9 THEN 'Float' -- Corrected Order
                WHEN SortOrder = 10 THEN 'Flag (2 Values)' -- Corrected Order
