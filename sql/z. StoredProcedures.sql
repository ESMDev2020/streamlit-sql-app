...
DECLARE @input NVARCHAR(MAX) = N'[OEDETAIL],[OEOPNORD], [PODETAIL],[SPHEADER]'
...
-- EXEC [mrs].[my_sp_translate_sql_query] @input_query = @input, @translated_query = @output OUTPUT;  -- translate with spaces
 ...
 EXEC [mrs].[usp_TranslateSQLQuery] @SQLQuery = @input,  @TranslatedQuery = @output OUTPUT, @DebugMode = 0, @Execution = 0; --, @Execution = 1; --V4
SELECT @output
