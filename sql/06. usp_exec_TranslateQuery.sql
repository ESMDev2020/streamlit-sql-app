...
--ROP Query - price
set @myvarInputSQL = ' SELECT [ITEMONHD].[IOITEM], [ITEMONHD].[IOBPRC] FROM [ITEMONHD] '
SELECT 
    [z_Item_on_Hand_File_____ITEMONHD].[ITEM_NUMBER_____IOITEM], 
    [z_Item_on_Hand_File_____ITEMONHD].[Base_Price_____IOBPRC] 
FROM [z_Item_on_Hand_File_____ITEMONHD]


-- Material Processing
--set @myvarInputSQL = 'SELECT [MPDETAIL].[MDORDR], [APVEND].[VVNDR], [APVEND].[VALPHA], [MPDETAIL].[MDCLOS], [MPDETAIL].[MDITEM], [MPDETAIL].[MDCRTD], [mdrqcc]*1000000+[mdrqyy]*10000+[mdrqmm]*100+[mdrqdd], [MPDETAIL].[MDOQTY], [MPDETAIL].[MDOUOM], [MPDETAIL].[MDIQTY] FROM [APVEND] [APVEND], [MPDETAIL] [MPDETAIL] WHERE [APVEND].[VCOMP] = [MPDETAIL].[MDCOMP] AND [APVEND].[VVNDR] = [MPDETAIL].[MDVNDR] ORDER BY [MPDETAIL].[MDITEM], [MPDETAIL].[MDORDR] DESC;'


--Material processing order detail
...
--ROP - QUERY -- ROP QUERY
--set @myvarInputSQL = '''SELECT ''ROP query'', [ITEMMAST].[IMFSP2], [ITEMMAST].[IMITEM], [ITEMMAST].[IMSIZ1], [ITEMMAST].[IMSIZ2], [ITEMMAST].[IMSIZ3], [ITEMMAST].[IMDSC2], [ITEMMAST].[IMWPFT], [ITEMONHD].[IOACST], [ITEMONHD].[IOQOH], [ITEMONHD].[IOQOO], [IOQOR]+[IOQOOR], [ITEMMAST].[IMDSC1], [ITEMMAST].[IMWUOM], [ITEMMAST].[IMCSMO]
...
--set @myvarInputSQL = '''SELECT ''Usage sum query'', DISTINCT [ITEMHIST].[IHITEM], [ITEMHIST].[IHTRNT], [ITEMHIST].[IHCUST], [ARCUST].[CALPHA], [ARCUST].[CSMDI1]*100 + [ARCUST].[CSLMN1]  
...
--SalesAnalisysQuery_
--Customer-Summary	
--set @myvarInputSQL = '''SELECT DISTINCT ''customer summary'',  [ARCUST].[CDIST]*100000 + [ARCUST].[CCUST], [ARCUST].[CALPHA], [ARCUST].[CLIMIT], [ARCUST].[CISMD1]*100 + [ARCUST].[CISLM1], [ARCUST].[CSMDI1]*100 + [ARCUST].[CSLMN1], [SALESMAN].[SMNAME]  
