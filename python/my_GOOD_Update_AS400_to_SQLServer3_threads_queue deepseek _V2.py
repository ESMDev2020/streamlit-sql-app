...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            # 1. Get tables from INFORMATION_SCHEMA (existing z_ tables)
            ...
            # 3. Get additional tables from mrs.01_AS400_MSSQL_Equivalents
            varListAdditionalTables = []
            try:
                varObjCursor.execute(
                    ...
                    f"WHERE MSSQL_TableName NOT LIKE 'z\\_%' ESCAPE '\\'"
                )
                ...
                for mssql_table, as400_table in varObjCursor.fetchall():
                    ...
                    varListAdditionalTables.append(standardized_name)
            except Exception as e:
                fun_PrintStatus("SYSTEM", f"Could not query 01_AS400_MSSQL_Equivalents: {str(e)}", "failure")
            
            # Combine and deduplicate the lists
            varListAllTables = list(set(varListMSSQLTablesfromSYS_SCHEMA + varListAdditionalTables))
            print(f"Found {len(varListAllTables)} tables to process ({len(varListMSSQLTablesfromSYS_SCHEMA)} from schema, {len(varListAdditionalTables)} from equivalents table)")
