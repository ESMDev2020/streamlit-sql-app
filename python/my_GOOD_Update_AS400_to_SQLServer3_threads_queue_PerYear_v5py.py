...
def fun_CompareAndSyncTables(varObjAs400Cursor, varObjMSSQLCursor, varStrAs400Table, varStrSqlTable):
    ...
    try:
        ...
        if not table_exists:
            ...
            # Build CREATE TABLE statement with basic type mapping
            create_sql = f"CREATE TABLE [{myDictConfig['SQL_SERVER']['SCHEMA']}].[{varStrSqlTable}] (\n"
            column_defs = []
            
            for col in columns:
                # Access columns by numeric index (0=COLUMN_NAME, 1=DATA_TYPE, 2=LENGTH)
                col_name = col[0].strip()  # Clean column names
                col_type = col[1]
                col_length = col[2] if len(col) > 2 else None
                
                # Clean problematic characters from column names
                clean_col_name = col_name.split('_____')[0]  # Take only part before _____
                clean_col_name = clean_col_name.replace('/', '_').replace('=', '_').replace('.', '_').replace(',', '_').replace(' ', '_')
                
                # Type mapping - adjust as needed
                if col_type in ['CHAR', 'VARCHAR']:
                    sql_type = f"VARCHAR({col_length})" if col_length else "VARCHAR(255)"
                ...
                column_defs.append(f"[{clean_col_name}] {sql_type}")
            ...
            try:
                ...
                varObjMSSQLCursor.execute(create_sql)
                varObjMSSQLCursor.connection.commit()  # Explicit commit after table creation
        ...
        def fun_Worker():
            while True:
                try:
                    ...
                    with varObjLogLock:
                        fun_PrintStatus("WORKER", f"Thread failed: {str(varExcError)}", "failure")
                    break
