...
CONFIG = {
    ...
    'MAX_THREADS': 20,    # Reduced from 20 to avoid connection pool exhaustion
    'MAX_RETRIES': 3,
    'RETRY_DELAY': 5,
...
def get_table_rowcount(cursor, schema, table):
    ...
    try:
        ...
        if schema == CONFIG['AS400']['LIBRARY']:
            # AS/400 syntax
...
def process_batch(sql_cursor, insert_sql, batch, table_name, retry_count=0):
    ...
    try:
        ...
        sql_cursor.connection.timeout = 60
...
def get_column_metadata(myCursor, mySchema, myTable):
    ...
    """)
    return [(col[0].strip(), (col[1] or "").strip().replace(" ", "_").replace("'", "_").replace(']', ']]')) for col in myCursor.fetchall()]
...
def process_table(table_info, log_lock, report_lock):
    ...
    try:
        ...
        #Verify.... we need to implement 
        # same rowcount = skip. Different = identify differences and process. 
        if as400_count == sql_count:
            myFunPrintStatus(as400_table, f"Skipping - counts match {as400_count} = {sql_count}", "success")
            return
        else:
            # Drop destination table if counts don't match
            myFunPrintStatus(sql_table, f"Dropping table - counts don't match: AS400={as400_count} vs SQL={sql_count}", "drop")
            sql_cursor.execute(f"DROP TABLE IF EXISTS [{CONFIG['SQL_SERVER']['SCHEMA']}].[{sql_table}]")
            sql_conn.commit()
        ...
        myFunPrintStatus(dest_table, "Creating destination table", "process")
        create_table_sql = f"""
        IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = '{dest_table}' AND schema_id = SCHEMA_ID('{CONFIG['SQL_SERVER']['SCHEMA']}'))
        BEGIN
            CREATE TABLE [{CONFIG['SQL_SERVER']['SCHEMA']}].[{dest_table}] (
                {', '.join([f'[{col[1]}_____{col[0]}] NVARCHAR(MAX)' for col in cols])}
            )
        END
        """
...
if __name__ == "__main__":
    try:
        ...
        # Process tables with controlled threading
        ...
        for table in tables:
            ...
            while len(active_threads) >= CONFIG['MAX_THREADS']:
