...
try:
    ...
    # ---------- Input moved below inventory table ----------
    item_id = st.text_input("Enter Item Number", value=st.session_state.selected_item)
    if st.button("Report by ITEM"):
        st.write(f"🔎 Generating report for Item: {item_id}...")
        try:
            with engine.begin() as conn:
                queries = [
                    f"SELECT [Size Text], Description, SMO FROM ROPData WHERE Item = {item_id}",
                    f"""
                    WITH Months AS (
                    ...
                    Aggregated AS (
                        SELECT FORMAT(DATEFROMPARTS(2000 + IHTRYY, IHTRMM, 1), 'yyyy-MM') AS Month,
                               CASE WHEN IHTRNT IN ('IA', 'OW', 'OR', 'IN') THEN IHTQTY ELSE 0 END AS Qty,
                               IHTRNT AS Type
                        FROM UsageData
                            WHERE CONVERT(VARCHAR, IHITEM) = '{item_id}'
                            AND DATEFROMPARTS(2000 + IHTRYY, IHTRMM, 1) >= DATEADD(MONTH, -13, CAST(GETDATE() AS DATE))
                    )
                    SELECT m.Month,
                           FORMAT(SUM(CASE WHEN a.Type = 'IA' THEN a.Qty ELSE 0 END), 'N0') AS [IA (Inventory Adjustments)],
                           FORMAT(SUM(CASE WHEN a.Type IN ('OW', 'OR') THEN a.Qty ELSE 0 END), 'N0') AS [MP (Material Processed)],
                           FORMAT(SUM(CASE WHEN a.Type = 'IN' THEN a.Qty ELSE 0 END), 'N0') AS [SO (Sales Orders)]
                    FROM Months m
                    LEFT JOIN Aggregated a ON m.Month = a.Month
                    GROUP BY m.Month
                    ORDER BY m.Month
                    """,
                    f"SELECT [OnHand], [Rsrv], [OnHand] - [Rsrv] AS [Available Inv] FROM ROPData WHERE Item = {item_id}",
                    f"SELECT [#/ft], [UOM], [$/ft], [con/wk], [Vndr] FROM ROPData WHERE Item = {item_id}",
                    f"""
                    SELECT ([OnHand] - [Rsrv]) AS [Available Inv],
                    ...
                    SELECT ropdata.Item, (ropdata.OnHand - ropdata.Rsrv) AS [Available], ropdata.OnPO, ropdata.[#/ft],
                           ropdata.[#/ft] + (
                               ...
                               WHERE IHITEM = ropdata.Item AND CAST(column4 AS VARCHAR(10)) = CAST(ropdata.Item AS VARCHAR(10))
                           ) AS net_consumption,
                ...
                result_sets = [pd.read_sql(query, conn) for query in queries]
