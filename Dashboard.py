...
def Sub_load_and_display_inventory_summary(engine):
    try:
        ...
        reorder_filter = st.selectbox("Filter by Reorder Flag", ["All"] + sorted(df["Reorder Flag"].dropna().unique()))
...
def Sub_AltairChartForFlag(filtered_df):
     # Altair Chart
...
def Sub_ShowInteractiveDataFrame():
...
def Sub_PieChartforOriginDistribution():
...
try: 
    ...
    filtered_df = Sub_load_and_display_inventory_summary(engine)

    #Display Altair chart for flags
    Sub_AltairChartForFlag(filtered_df)

    #Show interactive dataframe
    Sub_ShowInteractiveDataFrame()

    Sub_ReportperItem()

    Sub_PieChartforOriginDistribution()
